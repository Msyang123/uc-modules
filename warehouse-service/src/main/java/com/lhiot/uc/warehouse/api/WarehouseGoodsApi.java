...
public class WarehouseGoodsApi {
    ...
    @ApiImplicitParam(paramType = ApiParamType.PATH, name = "baseUserId", value = "基础用户id", required = true, dataType = "long")
    @RequestMapping(value = "/select/{baseUserId}", method = RequestMethod.GET)
    public ResponseEntity getWhGoods(@PathVariable("baseUserId") Long baseUserId) {
    ...
    public ResponseEntity<?> cancel(@RequestParam @NotNull @Min(1) String orderCode, @RequestParam String backCause) {
        ...
        warehouseGoodsService.cancelApplyWarehouseGoods(pagerResultObject.getArray(), orderCode, backCause);
    ...
    public ResponseEntity<?> confirm(@RequestParam @NotNull String orderCode) {
        ...
        //确认从仓库移除商品
        warehouseGoodsExtractService.confirmApplyWarehouseGoods(pagerResultObject.getArray());
        return ResponseEntity.ok(orderCode);
    }
}
